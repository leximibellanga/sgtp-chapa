import { ArrowBigLeft, BarChart3, Car, ClipboardList, LayoutDashboard, LogOut, Menu, PanelLeftClose, Users, Wallet } from "lucide-react";
import { useAuth } from "../../hooks/useAuth";
import { NavLink } from "react-router-dom";
import { useEffect, useState } from "react";

const links = [
    { to: "/admin/dashboard", label: "Dashboard", title: "Dashboard", icon: LayoutDashboard },
    { to: "/admin/carros", label: "Carros", title: "Carros", icon: Car },
    { to: "/admin/usuarios", label: "Usuarios", title: "Usuarios", icon: Users },
    { to: "/admin/registos", label: "Registos", title: "Registos", icon: ClipboardList },
    { to: "/admin/gastos", label: "Gastos", title: "Gastos", icon: Wallet },
    { to: "/admin/relatorios", label: "Relatorios", title: "Relatorios", icon: BarChart3 },
]
...
export default function Sidebar() {
    ...
    return (
        <aside className={`sticky ml-2 mt-2 mb-2 rounded-xl bg-[#001f18] text-white flex flex-col transition-all duration-450 overflow-hidden ${isOpen ? "w-60" : "w-16"}`}>
            <div className={`h-18 flex items-center ${isOpen ? "justify-between" : "justify-center"} px-3 border-b border-emerald-950`}>
                {isOpen && <h1 className="pl-3 text-xl font-bold">SGTP</h1>}
                <button
