...
export default function GastoFormModal({ open, onClose, onSuccess, gasto }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdicao ? "Editar gasto" : "Novo gasto"}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Select label={"Carro"} error={errors.carroId?.message} {...register("carroId")}>
                <option value="">Selecione o carro</option>
                {carros.map((c) => (
                    <option key={c.id} value={c.id}>{c.matricula} - {c.modelo}</option>
                ))}
            </Select>
